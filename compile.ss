...
(define (tmi-match-pattern target pat)
  (mtch pat
    ('identifier name . _)
      `(,target)
    ('integer value . _)
    ...
    ('string value . _)
      (if (equal? target (strip-quotes value))
        ...
        #f)
    ('constructor value . _)
      (if (equal? target (string->symbol value))
         '()
         #f)
    ('app (('constructor . cd) . ad))
      (tmi-match-app-pattern target `((constructor . ,cd) . ,ad))
    ('app (('identifier s . cd) var))
      (let ((predicate (eval (string->symbol (++ "t-" (string->symbol s) "?")))))
        (if (predicate target)
          `(,target)
          #f))))
      ;(tmi-match-app-pattern target `((identifier . ,cd) ,var))))
