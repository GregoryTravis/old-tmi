...
(define (tinf0 e env unis)
  (mtch e
    ...
    ;; g :: (a -> b) -> (c -> d) -> (c -> d)
    ;; Fixn 1 (f g) :: c -> d
    ('Fixn ('K 1) (PT FixList (f g)))
      (mtch (tinf0 f env unis)
        (('T f ('PT 'Fun (ab ('PT 'Fun (cd ab2))))) unis)
          (mtch (tinf0 g env unis)
            (('T g ('PT 'Fun (ab3 ('PT 'Fun (cd2 cd3))))) unis)
              `((T (Fixn (K 1) (PT FixList ((T ,f (PT Fun (,ab (PT Fun (,cd ,ab2)))))
                                            (T ,g (PT Fun (,ab3 (PT Fun (,cd2 ,cd3))))))))
                   ,cd)
                ((,ab ,ab2)
                 (,ab ,ab3)
...
(define (leval e env)
  (mtch e
    ...
    ('Fixn (K i) (PT FixList (f g)))
      (leval `(A (A ,(nth i `(,f ,g))
                    (L (V x) (A (Fixn 0 (,f ,g)) (V x))))
                 (L (V x) (A (Fixn 1 (,f ,g)) (V x))))
             env)
    ('V v)
      (lookup v env)
    ('K k)
