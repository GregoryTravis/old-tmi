...
(define (gather-tvars-1 e)
  (mtch e
    ...
    ('TP a b)
      (append (gather-tvars-1 a) (gather-tvars-1 b))
    ('TS a b)
      (append (gather-tvars-1 a) (gather-tvars-1 b))
    ('L v body)
...
(define (extend-env-with-pvs env e)
  (mtch e
    ('T ('PV x) t)
      `((,x . ,t) . ,env)
    ('T ('P a b) t)
      (extend-env-with-pvs (extend-env-with-pvs env a) b)
    ('T ('S a b) t)
      (extend-env-with-pvs (extend-env-with-pvs env a) b)
    ('T ('K k) ('C c))
      env))
;(tracefun extend-env-with-pvs)
  ...
  (mtch e
    ...
    ('L pat body)
      (mtch (tinf0 pat env unis)
        (('T pat pat-t) unis)
          (let ((env (extend-env-with-pvs env `(T ,pat ,pat-t))))
            (mtch (tinf0 body env unis)
              (('T body body-t) unis)
                `((T (L (T ,pat ,pat-t) (T ,body body-t)) (PT Fun (,pat-t ,body-t))) ,unis))))
    ('A fun arg)
      (let ((result-t (ty)))
    ...
    ('P a b)
      (let ((result-t (ty)))
        (mtch (tinf0 a env unis)
          (('T a a-t) unis)
            (mtch (tinf0 b env unis)
              (('T b b-t) unis)
                `((T (P (T ,a ,a-t) (T ,b ,b-t)) (TV ,result-t))
                  (((TV ,result-t) (TP ,a-t ,b-t))
                   . ,unis)))))
    ('S a b)
      (let ((result-t (ty)))
        (mtch (tinf0 a env unis)
...
(define (apply-unifiers-to-type-term unifiers term)
  (mtch term
    ...
    ('TP a b)
      `(TP ,(apply-unifiers-to-type-term unifiers a) ,(apply-unifiers-to-type-term unifiers b))
    ('TS a b)
      `(TS ,(apply-unifiers-to-type-term unifiers a) ,(apply-unifiers-to-type-term unifiers b))
    ))
...
(define (apply-unifiers-to-term unifiers e)
  (mtch e
    ...
    ('P a b)
      `(P ,(apply-unifiers-to-term unifiers a) ,(apply-unifiers-to-term unifiers b))
    ('S a b)
      `(S ,(apply-unifiers-to-term unifiers a) ,(apply-unifiers-to-term unifiers b))
    ('Fix fun)
...
; rebuild Nil = Nil
; rebuild (Cons a d) = Cons a (rebuild d)
; rebuild = Fix /. rec /./.
; Nil = Nil
; (P (K Cons) (P (V a) (V d))) = (P (K Cons) (P (V a) (A (V rec) (V d))))
(define foo
  '(Fix (L (V rec)
     (L (P (K Cons) (P (PV a) (PV d)))
        (P (K Cons) (P (V a) (A (V rec) (V d))))))))
