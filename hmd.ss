...
(define (pattern-match-pt-args-and-extend-env args-p args-x env)
  (mtch `(,args-p ,args-x)
    ((pa . pd) (xa . xd))
      (mtch (pattern-match-and-extend-env pa xa env)
        (env)
          (pattern-match-pt-args-and-extend-env pd xd env)
        '()
          '())
    ('() '())
      env))
  ...
  (mtch `(,p ,x)
    ...
    (('C ca) ('C cb))
      (if (eq? ca cb)
        `(,env)
        '())
    (('PT ctor-p args-x) ('PT ctor-p args-x))
      (if (and (eq? ctor-p ctor-x) (eq? (length args-p) (length args-x)))
        (pattern-match-pt-args-and-extend-env args-p args-x env)
        '())))
;(tracefun pattern-match-and-extend-env pattern-match-pt-args-and-extend-env)
